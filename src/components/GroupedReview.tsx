...
function GroupRow({ group, categories }: { group: PayeeGroupView; categories: CategoryOption[] }) {
  ...
  function apply(categoryId: string) {
    ...
    startTransition(async () => {
      try {
        await categorizeGroup(group.key, group.matchField, categoryId);
      } catch (e) {
    ...
    <tr className={`border-b border-rule align-top ${pending ? "opacity-50" : ""}`}>
      ...
      <td className="py-2">
        <select
          ...
          title={`Categorize all ${group.count} — creates a rule covering future ones too`}
        >
...
export function GroupedReview({
  ...
  return (
    <table className="w-full border-collapse">
      ...
      <tbody>
        {groups.map((g) => (
          <GroupRow key={g.key} group={g} categories={categories} />
        ))}
      </tbody>
    </table>
  );
